...
use Lemuria\Exceptions\IntentException;
use Lemuria\Factory\Factory;
use Lemuria\Http;
use Lemuria\HttpServer;
use Lemuria\WebSockets\Event;
use Lemuria\WebSockets\Handlers;
use Lemuria\WebSockets\Intents;
use Lemuria\WebSockets\Op;
use Lemuria\Helpers\Deferred;
use Lemuria\Drivers\React;
use Lemuria\Endpoint;
use Lemuria\Parts\Player\Client;
...
class Lemuria
{
    ...
    /**
     * Creates a Lemuria client instance.
     *
	...
	public function __construct(array $options = [])
    {
		if (php_sapi_name() !== 'cli') {
            trigger_error('Lemuria will not run on a webserver. Please use PHP CLI to run a Lemuria bot.', E_USER_ERROR);
        }
		...
		} else echo '[LEMURIA-WARNING] No discord set!' . PHP_EOL;
    ...
    {
        $this->logger->info('websocket reconnected to lemuria');
    ...
    protected function handleReady(object $data)
    {
        ...
        if ($this->reconnecting) {
            ...
            $this->logger->debug('websocket reconnected to lemuria through identify');
            $this->emit('reconnected', [$this]);
        ...
        $this->logger->debug('lemuria trace received', ['trace' => $content->_trace]);
    ...
    /**
     * Identifies with the Lemuria gateway with `IDENTIFY` or `RESUME` packets.
     *
            ...
            $payload = [
                ...
                'd' => [
                    ...
                    'properties' => [
                        ...
                        '$referrer' => 'https://github.com/VZGCoders/lemuria-bot',
                        '$referring_domain' => 'https://github.com/VZGCoders/lemuria-bot',
                    ],
                    'compress' => true,
    ...
    /**
     * Sends a heartbeat packet to the Lemuria gateway.
     */
    ...
    /**
     * Initializes the connection with the Lemuria gateway.
     */
    ...
    /**
     * Sends a packet to the Lemuria gateway.
     *
    ...
    protected function setGateway(?string $gateway = null): ExtendedPromiseInterface
    {
        ...
        if (is_null($gateway)) {
            $this->http->get(Endpoint::GATEWAY_BOT)->done(function ($response) use ($buildParams) {
                ...
                $buildParams('wss://lemuria.valzargaming.com');
            });
        } else {
            $buildParams($gateway);
        }
	...
	*/
	...
	{
		if ($this->verbose) $this->emit('[LEMURIA] [RESOLVE OPTIONS]');
    ...
    {
        $this->closing = true;
        $this->ws->close(Op::CLOSE_UNKNOWN_ERROR, 'lemuria closing...');
        $this->emit('closed', [$this]);
        $this->logger->info('lemuria closed');
     ...
     * @return Http
     *
     * @deprecated Use Lemuria::getHttpClient()
     */
