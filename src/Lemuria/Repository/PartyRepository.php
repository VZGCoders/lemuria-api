...
use Lemuria\Parts\Party\Party;
use Lemuria\Parts\Part;
use React\Promise\ExtendedPromiseInterface;

/**
 * Contains all parties in Lemuria.
 *
 * @see \Lemuria\Parts\Party\Party
 *
...
class PartyRepository extends AbstractRepository
{
    ...
    {
		if ($this->factory->lemuria->parties->offsetGet($part->id)) $method = 'patch';
		else $method = 'post';
		...
		return $this->factory->lemuria->browser->post($url, ['Content-Type' => 'application/json'], json_encode($part))->then( //Make this a function
			function ($response) use ($part) {
				echo '[SAVE RESPONSE] '; //var_dump($this->factory->lemuria->parties->offsetGet($part->id)); 
				//var_dump($this);
			},
			function ($error) {
	...
	public function delete($part): ExtendedPromiseInterface
	{
		if (! ($part instanceof Part)) {
            $part = $this->factory->lemuria->parties->offsetGet($part);
        }
		...
		return $this->factory->lemuria->browser->get($url)->then( //Make this a function
			function ($response) { //TODO: Not receiving response
				...
				*/
				...
				//var_dump($that->factory->lemuria->parties);
				return $this;
			},
			function ($error) {
	...
	/**
     * Creates a new Party for the repository and saves it to the Lemuria servers.
	...
	public function new($player = null, $party = null, $name = null): string
	{
		...
		if ($name) $this->save($this->factory->lemuria->factory(\Lemuria\Parts\Party\Party::class, ['player1' => $player->id, 'name' => $name]));
		else $this->save($this->factory->lemuria->factory(\Lemuria\Parts\Party\Party::class, ['player1' => $player->id]));
		/*
			...
			function($result) use ($lemuria, $message, $player) {
				if (count($collection = $this->filter(fn($p) => $p->player1 == $player->id))>0) {
					foreach ($collection as $party) { //There should only be one
						...
						$lemuria->players->save($player)->done(
		...
		*/
		return 'Your Party is currently being created. You can retrieve it with `<@' . $this->factory->lemuria->discord->id . '>party` in a few moments.';
	}
	...
	*/
	public function join($player = null, $party = null, $id = null): string
	{
		...
		return $party->join($this->factory->lemuria, $player);
	}
	...
	*/
	...
	{		
		...
		if ($player_part instanceof Player) {
			...
			if (! $player = $this->factory->lemuria->players->offsetGet($player_part)) return false;
		} else return false;
		...
		$player->party_id = $party->id;
		$this->factory->lemuria->players->save($player);
		$this->save($party);
		return true;
    }
    ...
    */
    public function kick($player_part): null|bool
    {
		...
		elseif (is_numeric($player_part)) {
			if (! $player = $this->factory->lemuria->players->offsetGet($player_part)) return null;
			if (! $player->party_id) return null;
			if (! $party = $this->offsetGet($player->party_id)) return null;
		} else return null;
	...
	*/
	public function disband($part): null|bool
	{
		...
		$lemuria->parties->delete($party)->done(
	...
	*/
	function isPartyFull($part = null): null|bool
	{
		if ($part instanceof Party) {
			...
			if (! $party = $this->factory->lemuria->parties->offsetGet($id)) return null;
		} elseif (is_numeric($part)) {
			if (! $party = $this->factory->lemuria->parties->offsetGet($part)) return null;
		} else return null;
	...
	*/
	function partyEmbed($id)
	{
		...
		$players = array();
		$players[] = $player1 = $this->factory->lemuria->players->offsetGet($party->player1);
		$players[] = $player2 = $this->factory->lemuria->players->offsetGet($party->player2);
		$players[] = $player3 = $this->factory->lemuria->players->offsetGet($party->player3);
		$players[] = $player4 = $this->factory->lemuria->players->offsetGet($party->player4);
		$players[] = $player5 = $this->factory->lemuria->players->offsetGet($party->player5);
		
		$embed = $this->factory->lemuria->discord->factory(\Discord\Parts\Embed\Embed::class);
		$embed->setColor(0xe1452d)
		//	->setDescription('$author_guild_name') // Set a description (below title, above fields)
		//	->setImage('https://avatars1.githubusercontent.com/u/4529744?s=460&v=4') // Set an image (below everything except footer)
			->setTimestamp()
			->setFooter('Lemuria by ArtsyAxolotl#5128')                             					// Set a footer without icon
			->setURL('');                             												// Set the URL
		if ($party->name) $embed->addFieldValues('Name', $party->name, true);
		$embed->addFieldValues('ID', $party->id, true);
		foreach ($players as $player) {
			if ($player && $user = $this->factory->lemuria->discord->users->offsetGet($player->discord_id)) {
