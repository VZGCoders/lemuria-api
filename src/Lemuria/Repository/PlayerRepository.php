...
/**
 * Contains all Players in Lemuria.
 *
 * @see \Lemuria\Parts\Player\Player
 *
...
class PlayerRepository extends AbstractRepository
{
	...
	/**
     * Creates a new Player for the repository and saves it to the Lemuria servers.
    ...
    {
		$snowflake = \Lemuria\generateSnowflake($this->factory->lemuria);
		if (! $account = $lemuria->accounts->get('discord_id', $discord_id)) {
			//
		...
		if ($part = $this->factory->lemuria->factory(\Lemuria\Parts\Player\Player::class, [
			'id' => $snowflake,
			...
			if (! $species || ! in_array(trim($species), $part::getFillableSpeciesAttributes())) {
				$return = 'Please tell us the species you want for your Player in the following format: `<@' . $this->factory->lemuria->discord->id . '> player create {species}` where `{species}` is any of the following:' . PHP_EOL;
			...
			$this->save($part);
			return 'Created player `' . $part->name . ' ` with ID `' . $part->id . '`. You can make this your active Player by using the command `<@' . $this->factory->lemuria->discord->id . '> player activate ' . $part->id . '`.';				
    ...
    public function activate($discord_id = null, $id = null): string
    {
		if (! ($id instanceof Player)) {
			if (! is_numeric($id)) return "You must include the numeric ID of the Player you want to activate! You can check `<@{$this->factory->lemuria->discord->id}> players` if you need a list of your IDs!";
		...
		if (! $discord_id) return $part->activate($this->factory->lemuria);
		if ($part->account_id) {
			if ($account = $this->factory->lemuria->accounts->offsetGet($part->account_id))
				if ($account->discord_id == $discord_id) return $part->activate($this->factory->lemuria);
		}
	...
	*/
	function playerEmbed($id)
	{
		...
		if ($party = $this->factory->lemuria->parties->offsetGet($player->party_id))
		...
		if ($account = $this->factory->lemuria->accounts->offsetGet($player->account_id)) {
			if ($user = $this->factory->lemuria->discord->users->offsetGet($account->discord_id)) {
