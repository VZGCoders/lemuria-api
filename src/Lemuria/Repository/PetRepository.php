...
use Lemuria\Http;
use Lemuria\Parts\Pet\Pet;
use Lemuria\Parts\Part;
use React\Promise\ExtendedPromiseInterface;

/**
 * Contains all Pets in Lemuria.
 *
 * @see \Lemuria\Parts\Pet\Pet
 *
...
class PetRepository extends AbstractRepository
{
	...
	/**
     * Creates a new Pet for the repository and saves it to the Lemuria servers.
    ...
    public function new($author_id = null, $species = null, $name = null, $class = null): string
    {
		...
		if ($part = $this->factory->lemuria->factory(\Lemuria\Parts\Pet\Pet::class, [
			'id' => $snowflake,
			...
			if (! $species || ! in_array(trim($species), $part::getFillableSpeciesAttributes())) {
				$return = 'Please tell us the species you want for your Pet in the following format: `<@' . $this->factory->lemuria->discord->id . '> pet create {species}` where `{species}` is any of the following:' . PHP_EOL;
			...
			$this->save($part);
			return 'Created pet `' . $part->name . ' ` with ID `' . $part->id . '`. You can make this your active Pet by using the command <@' . $this->factory->lemuria->discord->id . '> pet activate ' . $part->id . '`.';				
    ...
    public function activate($author_id = null, $id = null): string
    {
		if (! ($id instanceof Pet)) {
			if (! is_numeric($id)) return "You must include the numeric ID of the Pet you want to activate! You can check `<@{$this->factory->lemuria->discord->id}> pets` if you need a list of your IDs!";
		...
		if (! $author_id) return $part->activate($this->factory->lemuria);
		if ($part->discord_id == $author_id) return $part->activate($this->factory->lemuria);
