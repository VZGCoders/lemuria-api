...
use Lemuria\Http;
use Lemuria\Parts\Battle\Battle;
use Lemuria\Parts\Part;
use React\Promise\ExtendedPromiseInterface;

/**
 * Contains all battles in Lemuria.
 *
 * @see \Lemuria\Parts\Battle\Battle
 *
...
class BattleRepository extends AbstractRepository
{
    ...
    {
		if ($this->factory->lemuria->battles->offsetGet($part->id)) $method = 'patch';
		else $method = 'post';
		...
		return $this->factory->lemuria->browser->post($url, ['Content-Type' => 'application/json'], json_encode($part))->then( //Make this a function
			function ($response) use ($part) {
				echo '[SAVE RESPONSE] '; //var_dump($lemuria->battles->offsetGet($part->id)); 
				//var_dump($lemuria->battles);
			},
			function ($error) {
	...
	public function delete($part): ExtendedPromiseInterface
	{
		if (! ($part instanceof Part)) {
			$part = $this->factory->lemuria->battles->offsetGet($part);
        }
		
		$url = Http::BASE_URL . "/battles/delete/{$part->id}/";
		return $this->factory->lemuria->browser->post($url, ['Content-Type' => 'application/json'], json_encode($part))->then( //Make this a function
			function ($response) use ($part) {
				echo '[DELETE RESPONSE] '; //var_dump($lemuria->battles->offsetUnset($part->id)); 
				//var_dump($lemuria->battles);
			},
			function ($error) {
    ...
    public function fresh(Part $part): ExtendedPromiseInterface
    {
		...
		return $this->factory->lemuria->browser->post($url, ['Content-Type' => 'application/json'], json_encode($part))->then( //Make this a function
			function ($response) use ($lemuria, $message, $part) {
				echo '[FETCH RESPONSE] '; //var_dump($lemuria->battles->offsetUnset($part->id)); 
				//var_dump($lemuria->battles);
			},
			function ($error) {
		...
		return $this->factory->lemuria->browser->get($url)->then( //Make this a function
			function ($response) { //TODO: Not receiving response
				...
				*/
				...
				//var_dump($that->factory->lemuria->battles);
				return $this;
			},
			function ($error) {
