...
use Symfony\Component\OptionsResolver\Options;
 ...
 /**
 * The Lemuria EPOCH, the first second of 2021 (GMT -4).
 *
 * @int Lemuria EPOCH.
 */
 ...
 * @return int
 */
function generateSnowflake(Lemuria $lemuria, $time = null, $workerID = null, $processID = null, $increment = null)
{
	$processID = $processID ?? $lemuria->processID;
	$workerID = $workerID ?? $lemuria->workerID;
	$increment = $increment ?? $lemuria->increment;
	$lemuria->increment++;
 ...
 * @return Part
 */
function partPusher($lemuria, $repository, $part_name, $array)
{
	$part = null;
...
function getCurrentPlayer($lemuria, $account_id)
{
	if (count($collection = $lemuria->players->filter(fn($p) => $p->account_id == $account_id && $p->active == 1 )) > 0) {
	...
	if (! empty($get)) {
		$part = partPusher($lemuria, 'players', '\Lemuria\Parts\Player\Player', $get);
		echo '[getCurrentPlayer2]'; var_dump($part);
		return $part;
	} else return null;
}
function getCurrentParty($lemuria, $id)
{
	if (count($collection = $lemuria->parties->filter(fn($p) => $p->player1 == $id || $p->player2 == $id || $p->player3 == $id || $p->player4 == $id || $p->player5 == $id))>0) {
	...
	if ($stmt = $PDO->prepare($sql))
		if ($stmt->execute([$id]))
			if ($result = $stmt->fetchAll())
				$part = partPusher($lemuria, 'parties', '\Lemuria\Parts\Party\Party', $result);
	return $part ?? false;
}
function getPlayerLocation($lemuria)
{
	//
}
