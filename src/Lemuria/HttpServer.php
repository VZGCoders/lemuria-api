...
class HttpServer
{
	public function __construct($lemuria, $socket)
    {
		$this->lemuria = $lemuria;
		$this->socket = $socket;
		
        $this->webapi = new \React\Http\HttpServer($lemuria->getLoop(), function (\Psr\Http\Message\ServerRequestInterface $request) use ($lemuria) {
			$discord = $lemuria->discord;
			try {
				echo '[API] ';
				...
				*/
				...
				$lemuriaBattleground = $lemuria->discord->getChannel("887118621065768970");
				$lemuriaBotSpam = $lemuria->discord->getChannel("887118679697940481");
				...
				if ($ver != 'v' . $lemuria::GATEWAY_VERSION) {
				...
				switch ($repository) { //gateway
					...
					case 'gateway':
						if ($method != 'bot') break;
						$return = array();
						$return['url'] = 'https://lemuria.valzargaming.com/gateway/';
						$return['shards'] = 1;
				...
				switch ($sub) {
					case 'ping':
						echo '[PING]' . PHP_EOL;
						$lemuriaBotSpam->sendMessage('Pong!');
						$return = 'Pong!';
						break;
					/*case 'stats':
						if ($embed = $stats->handle()) {
							$lemuriaBotSpam->sendEmbed($embed);
							$return = $embed;
						} else return webapiFail('stats', $stats);
						break;*/
					case 'channel':
						if (! $id || !webapiSnow($id) || ! $return = $lemuria->discord->getChannel($id))
							return webapiFail('channel_id', $id);
						break;

					case 'guild':
						if (! $id || !webapiSnow($id) || ! $return = $lemuria->discord->guilds->offsetGet($id))
							return webapiFail('guild_id', $id);
						break;
						
					case 'bans':
						if (! $id || !webapiSnow($id) || ! $guild = $lemuria->discord->guilds->offsetGet($id))
							return webapiFail('guild_id', $id);
						$return = $guild->bans;
						break;
					...
					case 'user':
						if (! $id || !webapiSnow($id) || ! $return = $lemuria->discord->users->offsetGet($id)) {
							return webapiFail('discord_id', $id);
						}
						break;

					case 'userName':
						if (! $id || ! $return = $lemuria->discord->users->get('name', $id))
					...
					case 'lookup':
						...
						if (! $id || !webapiSnow($id) || ! $return = $lemuria->discord->user->fetch($id))
							return webapiFail('discord_id', $id);
						break;
					...
					case 'owner':
						...
						if (! $id || !webapiSnow($id))
							return webapiFail('discord_id', $id);
						$return = false;
						if ($user = $lemuria->discord->users->offsetGet($id)) { //Search all guilds the bot is in and check if the user id exists as a guild owner
							foreach ($lemuria->discord->guilds as $guild) {
					...
					case 'avatar':
						...
						if (! $user = $lemuria->discord->users->offsetGet($id)) {
							$lemuria->discord->users->fetch($id)->done(
								function ($user) {
									$return = $user->avatar;
					...
					case 'avatars':
						...
						$return = [];
						$promise = $lemuria->discord->users->fetch($idarray[0])->then(function ($user) use (&$return) {
						  $return[$user->id] = $user->avatar;
						});
						
						for ($i = 1; $i < count($idarray); $i++) {
						  $promise->then(function () use (&$return, $idarray, $i, $discord) {
							return $lemuria->discord->users->fetch($idarray[$i])->then(function ($user) use (&$return) {
							  $return[$user->id] = $user->avatar;
							});
						  });
						}
				...
				$repositories = [
					'accounts' => [
						'part_name' => '\Lemuria\Parts\Account\Account',
					...
					'players' => [
						'part_name' => '\Lemuria\Parts\Player\Player',
					...
					'pets' => [
						'part_name' => '\Lemuria\Parts\Pet\Pet',
						'part_name_short' => 'Pet',
						'allowed_methods' => [
							['method' => 'get', 'privileged' => false, 'privileged_endpoints' => [null, 'all', 'freshen']],
							['method' => 'fresh', 'privileged' => false, 'privileged_endpoints' => []],
					...
					'npcs' => [
						'part_name' => '\Lemuria\Parts\NPC\NPC',
						'part_name_short' => 'NPC',
						'allowed_methods' => [
							['method' => 'get', 'privileged' => false, 'privileged_endpoints' => [null, 'all', 'freshen']],
							['method' => 'fresh', 'privileged' => false, 'privileged_endpoints' => []],
					...
					'parties' => [
						'part_name' => '\Lemuria\Parts\Party\Party',
					...
					'battles' => [
						'part_name' => '\Lemuria\Parts\Battle\Battle',
					...
					'votes' => [
						'part_name' => 'Lemuria\Parts\Vote\Vote',
				...
				if (in_array($method, $requires_part)) {
					...
					if ($attributes = json_decode(json_encode($data), true)) {
						...
						if ($part = $lemuria->factory($part_name, $attributes)) $has_part = true;
					...
					if ($target_method == 'patch') {
						...
						elseif ($update = sqlUpdate($fillable_attributes, array_values($attributes), $repository, 'id', $id2)) {
							...
							foreach ($get as $array) {
								$part = $lemuria->factory($part_name, $array);
								if ($part->id) {
									if ($lemuria->$repository->offsetGet($part->id))
										$lemuria->$repository->pull($part->id);
									$lemuria->$repository->push($part);
								}
							}
					...
					elseif ($target_method == 'post' || $target_method == 'put') { //Put works here because we should never be creating duplicates of objects or reusing the same id for multiple objects
						if ($id2 && is_numeric(($id2))) {
							...
							if (sqlCreate($repository, $data)) //Create a new part without using an ID
								echo '[CREATED PART WITHOUT ID]' . PHP_EOL; //$lemuria->$repository->freshen(); //attempt to retrieve the new part by freshening the repository
					...
					if ($id2 && is_numeric(($id2))) {
						if (in_array($partial, $allowed_properties)) {
							...
							if (!empty($array = sqlGet(['*'], $repository, 'id', [$id2], '', 1))) {
								...
								if ($attributes = json_decode(json_encode(json_validate($array)), true)) {
									if ($part = $lemuria->factory($part_name, $attributes[$id2])) {
										if ($lemuria->$repository->offsetGet($part->id))
											$lemuria->$repository->pull($part->id);
										$lemuria->$repository->push($part);
									}
						...
						foreach ($array as $data) //Create all into parts and push
							if ($attributes = json_decode(json_encode(json_validate($data)), true)) {
								if ($part = $lemuria->factory($part_name, $attributes)) {
									if ($lemuria->$repository->offsetGet($part->id))
										$lemuria->$repository->pull($part->id);
									$lemuria->$repository->push($part);
								}
				...
				elseif ($target_method == 'delete') { echo '[DELETE RESPONSE]' . PHP_EOL;
					...
					elseif (sqlDelete($repository, 'id', [$id2], '', 1)) {
						if ($lemuria->$repository->offsetGet($id2))
							$lemuria->$repository->pull($id2);
						return new \GuzzleHttp\Psr7\Response(200, ['Content-Type' => 'application/json'], json_encode($part ?? $lemuria->$repository->pull($id2) ?? $id2));
					} else return new \GuzzleHttp\Psr7\Response(400, ['Content-Type' => 'application/json'], json_encode($_400)); //The data provided is either missing or didn't get passed to the SQL method
				}
