...
class Player extends Part
{
	...
	public function activate($lemuria): string
	{
		$collection = $lemuria->players->filter(fn($p) => $p->account_id == $this->account_id && $p->active == 1);
		...
		$this->active = 1;
		$lemuria->players->save($this)->done(
	...
	public function deactivate($lemuria): string
	{
		$collection = $lemuria->players->filter(fn($p) => $p->account_id == $this->account_id && $p->active == 1 && $p->id != $this->id);
	...
	public function looking($lemuria): string
	{
	...
	public function rename($lemuria, $name): string
	{
		...
		if ($lemuria) $lemuria->players->save($this);
		return $return;
	}
